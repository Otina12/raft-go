...
// adapted from Go net/myrpc/server.go.
//
// sends mygob-encoded values to ensure that RPCs don't include references to program objects.
//
...
// Concurrent calls to Call() may be delivered to the server out of order, since the network may reorder messages.
...
// as pointers, so that their types exactly match the types of the arguments to Call().
//
// srv := MakeServer()
...
// false means that no reply was received from the server.
